...
def get_policy(checkpoint, output_dir, device):
    ...
    # load checkpoint
    payload = torch.load(open('./policy/R3DP/'+checkpoint, 'rb'), pickle_module=dill)
    cfg = payload['cfg']
    ...
    # cfg.policy.noise_scheduler.pop("variance_type")
...
class DP:
    def __init__(self, task_name, head_camera_type: str, checkpoint_num: int, expert_data_num: int, seed: int):
        self.policy = get_policy(f'checkpoints/{task_name}_{head_camera_type}_{expert_data_num}_{seed}_feat_only/{checkpoint_num}.ckpt', None, 'cuda:0')
    ...
    def clear_state(self):
        self.runner.clear_state(self.policy)
...
def main(usr_args):
    ...
    dp.policy.obs_encoder.set_tau(args['tau'])

    st_seed, suc_num = test_policy(task_name, task, args, dp, st_seed, test_num=test_num)
    suc_nums.append(suc_num)

    topk_success_rate = sorted(suc_nums, reverse=True)[:topk]
    save_dir = Path(f'eval_result/r3dp_tau{args["tau"]}/{task_name}_{usr_args.head_camera_type}/{usr_args.expert_data_num}')
